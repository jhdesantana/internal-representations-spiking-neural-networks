...
def calculate(D):
    if np.abs(pci_norm_factor(D)) > 0.001:
        return lz_complexity_2D(D) / pci_norm_factor(D)
    else:
        return 0

    # return lz_complexity_2D(D) / pci_norm_factor(D) if pci_norm_factor(D) != 0 else 0
...
def pci_norm_factor(D):
    ...
    if p0 == 0 or p1 == 0:
        ...
        H = -p1 * np.log2(p1) - p0 * np.log2(p0)
    # H = (0
    #      if math.isnan(-p1 * np.log2(p1) - p0 * np.log2(p0))
    #      else -p1 * np.log2(p1) - p0 * np.log2(p0)
    #      )
